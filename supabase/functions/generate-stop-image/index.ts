...
serve(async (req) => {
  ...
  try {
    const { stopName, stopType, stopDescription, city } = await req.json();

    if (!stopName || !stopType) {
      return new Response(
        JSON.stringify({ error: "Missing stop details" }),
    ...
    if (!aiResp.ok) {
      ...
      console.error("Image gen error:", aiResp.status, errText);
      ...
      throw new Error(`Image gen error [${aiResp.status}]`);
    }
    ...
    console.error("generate-stop-image error:", e);
    return new Response(
